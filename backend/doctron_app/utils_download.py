...
def create_json_to_download(annotation_type,annotators,batch,name_space,document,collection):
    ...
    if annotation_type == 'mentions' or annotation_type == 'all':
        print('mentions',len(users_list),len(documents))
    ...
    if annotation_type == 'concepts' or annotation_type == 'all':
        # json_to_download['annotation_type'] = 'concepts'
        print('concepts',len(users_list),len(documents))
    ...
    if annotation_type == 'tags' or annotation_type == 'all':
        # json_to_download['annotation_type'] = 'concepts'
        json_to_download['tags'] = []
        for u in users_list:
            for d in documents:
                annotations = AssociateTag.objects.filter(username=u, name_space=name_space, document_id=d)
                for annotation in annotations:
                    json_ann = {}
                    json_ann['username'] = u.username
                    json_ann['document_id'] = d.document_id
                    ...
                    json_to_download['tags'].append(json_ann)
    if annotation_type == 'labels' or annotation_type == 'all':
        # json_to_download['annotation_type'] = 'labels'
        print('labels',len(users_list),len(documents))
    ...
    if annotation_type == 'relationships' or annotation_type == 'all':
    ...
    if annotation_type == 'assertions' or annotation_type == 'all':
        # json_to_download['annotation_type'] = 'assertions'
        json_to_download['assertions'] = []
        print('assertions',len(users_list),len(documents))
...
def create_csv_to_download(annotation,annotators,batch,name_space,document,collection):
    ...
    try:
        ...
        if annotation == 'labels':
            ...
            row_list.append(['username','collection_name','document_id','batch','start','stop','mention_location','mention_text','tag','insertion_time'])
            for u in users_list:
                for d in documents:
                    annotations = AssociateTag.objects.filter(username=u, name_space=name_space, document_id=d)
                    for annotation in annotations:
