...
export default function AutoCompleteWithAdd(props) {
    ...
    function updateConceptsListsUrls(newValue) {
        ...
        ConceptsList.map(x => {
            ...
            if (x.url === newValue.url) {
                ...
                if(ConceptValueFinal !== undefined && ConceptValueFinal !== null){
                    if(x.name === ConceptValueFinal.name){
                        filtered = options.filter(ele => ele.area === x['area'] );
                        if (filtered.length === 0) {
                            options.push(obj)
                        }
                    }
    ...
    return (
        <div>
            ...
            <Autocomplete
                ...
                filterOptions={(options, params) => {
                    ...
                    if (inputValue !== '') {
                        ...
                        if (inputValue !== '' && !props.no_add_new_concept) {
                            filtered.push({
                                inputValue,
                                name: `Add "${inputValue}"`,
                            });
            ...
            />
            <br/>
            {ConceptValueFinal !== null && UrlValueFinal !== null && <div>
                <b>Concept description: </b>
                <p>
                    {ConceptsList.filter(x=>x.name === ConceptValueFinal.name && x.url === UrlValueFinal.url && x.description &&  x.description !== "None" ).length > 0 && <div style={{lineHeight:'1.5',fontSize:'0.8rem',padding:'1%'}}>{ConceptsList.filter(x=>x.name === ConceptValueFinal.name && x.url === UrlValueFinal.url && x.description &&  x.description !== "None" )[0].description}</div>}
                {/*{ConceptsList.map(x => <>*/}
                {/*    {x.name === ConceptValueFinal.name && x.url === UrlValueFinal.url && x.description &&  x.description !== "None" && <div style={{lineHeight:'1.5',fontSize:'0.8rem',padding:'1%'}}>{x.description}</div>}*/}
            ...
            <div style={{marginBottom: '10px'}}><span className={'urlnotmand'}
                                                      onClick={() => SetShowUrlComplete(prev => !prev)}>Select the URI</span>
            </div>
