...
export default function RelMentionOver(props){
    ...
    function update(mention,color_0){
        ...
        }else if(SourceAll.filter(item => mentions.includes(item)).length > 0){
        ...
        } else if(PredicateAll.filter(item => mentions.includes(item)).length > 0){
            let found_source = true
        ...
        }else if(TargetAll.filter(item => mentions.includes(item)).length > 0){
            let found_source = true
    ...
    },[props.mention,Source,Predicate,Target,SourceAll,PredicateAll,TargetAll])
