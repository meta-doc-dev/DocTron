...
export default function AnnotateAllModal(props) {
    ...
    function annotateAll(e,mention,position){
        e.preventDefault()
        e.stopPropagation()
        console.log(position)
        SetLoading(true)
        axios.post('mentions/annotate_all',{start:SelectedMention.start,position:position,stop:SelectedMention.stop,mention_text:SelectedMention.mention_text})
            .then(response=>{
    ...
    return (
        ...
        >
            ...
            <DialogTitle id="alert-dialog-title">
                Annotate all mentions
            </DialogTitle>
            {Loading === false && <DialogContent>
                {SelectedMention  ? <DialogContentText id="alert-dialog-description">
                        Annotate all the occurrences of the mention <b>{SelectedMention.mention_text}</b> and its linked concepts and tags (if any) occurring in this document?
                        If some concepts were already linked to these mentions, they will be overwritten.
                </DialogContentText> :
                    <DialogContentText id="alert-dialog-description">
                        Which mention do you want to select?
                        <div>
            ...
            {Loading === true && <DialogContent>Annotating. Please wait</DialogContent>}
            {Loading === false && <DialogActions>
                <Button onClick={(e) => {
                    handleClose(e)
                }}>No</Button>
                {(MentionsInvolved.length === 1 || SelectedMention) ?
                    <Button onClick={(e) => annotateAll(e, SelectedMention, props.position)}>Confirm</Button> :
                    <Button onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation()
