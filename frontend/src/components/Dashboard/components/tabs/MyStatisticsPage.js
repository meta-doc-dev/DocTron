...
import AgreementSortableTable from "@components/Dashboard/components/sortableTable/AgreementSortableTable";
import AgreementDocTable from "@components/Dashboard/components/sortableTable/AgreementDocTable";
...
const MyStatisticsPage = () => {
    ...
    const agreementColumns = [
        {key: "topic_id", label: "Topic ID", type: COLUMN_TYPES.TOPIC_ID},
        {key: "topic_title", label: "Topic", type: COLUMN_TYPES.TOPIC_TITLE},
        {key: "total_documents", label: "Total annotations", type: COLUMN_TYPES.ANNOTATED},
        {key: "total_annotators", label: "Annotators", type: COLUMN_TYPES.ANNOTATORS}
    ];
    ...
    const renderAgreementTable = (data, statType = "individual", withModals = true) => {
        return (
            <AgreementSortableTable
                data={data}
    ...
    const renderDocumentAgreementTable = (data, statType = "individual") => {
        if (statType === "global") return (
                <AgreementDocTable
                    data={data}
                    onBack={onBack}
    ...
    return (
        <section className="main__content my-stats">
            ...
            {statsActiveTab === "inter-agreement" && !loading && !error && indStatistics && (
                selectedTopic && documentStats ? (
                    renderDocumentAgreementTable(documentStats, "global")
                ) : (
                    renderAgreementTable(globalStatistics, "global")
                )
