...
export default function AssertionsList(props){
    const { collection,inarel,document_id,newfact,annotationtypes,snackmessage,opensnack,curannotator,modality,view,username,mentions,newfactin,modifyrel,targettext,predicatetext,sourcetext,targetconcepts,predicateconcepts,predicate,target,source,relationship,relationshipslist,sourceconcepts } = useContext(AppContext);
    ...
    function deleteRelation(e,relation){
        ...
        }else if(AnnotationTypes.indexOf('Facts annotation') === -1){
                SetOpenSnack(true)
                SetSnackMessage({'message': 'Facts annotation is not allowed here'})
        }else {
            axios.delete('relationships/delete', {
                data: {
                    source: source,
                    predicate: predicate,
                    target: target
    ...
    useEffect(()=>{
        axios.get('/get_assertions',{params:{user:CurAnnotator}}).then(response=>SetAssertions(response.data['assertions']))
    },[RelationshipsList])
