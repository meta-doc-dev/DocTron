...
export default function RelationshipsClass(props){
    const { modality,inarel,document_id,username,curannotator,snackmessage,opensnack,mentions,showrelspannel,readonlyrelation,modifyrel,targettext,predicatetext,sourcetext,targetconcepts,predicateconcepts,predicate,target,source,relationship,relationshipslist,sourceconcepts } = useContext(AppContext);
                            ...
                            <div style={{marginTop:'15px',paddingLeft:'5%'}}>
                                ...
                                {PredicateFilteredRelations[area].map((rel,i)=>
                                    <>
                                        ...
                                        }} className={OpenRelation === 'predicate' + '_' + area + '_' + i.toString() ? 'selected_item':''}>{rel['anchor'] !== '' ? rel['anchor'] : 'Relation ' + i.toString()}</span>
