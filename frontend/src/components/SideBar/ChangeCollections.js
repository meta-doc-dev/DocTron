...
export default function CollectionsComponent(props){
    ...
    return(
        <div>
            ...
            <div>
                {!CollList && <div className='loading'>
                    <CircularProgress/>
                </div>}
                                ...
                                <ThemeProvider theme={labelstheme}>
                                    {/*{c.id === Collection && <i>Current: </i>}*/}
                                        ...
                                        {c.perc_annotations_user > 20 && c.perc_annotations_user <= 40 && <div id={c.id} >
                                            <Chip label={c.name + ' ' + c.perc_annotations_user + '%'} color={'notempty'} variant={c.id === Collection ? "filled" : "outlined"} onClick={(e)=>handleClick(e,c.id)}/></div>}
