...
export default function FilterDocumentComponent({displayDocs, setDisplayDocs}){
    const { role,document_id,labels, inarel,labelstosave,collectiondocuments,annotatedlabels } = useContext(AppContext);
...
/*        axios.get('collections/documents')
    ...
    function get_filtered_list(filed,batch = null,ID = null){
        ...
        if (final_list.length > 0 && final_list.indexOf(DocumentID) === -1 && batch === null && ID === null){
            axios.post('update_document_id',{'document':final_list[0]['hashed_id']})
                .then(resp=>{
                    SetDocumentID(final_list[0]['hashed_id'])
                })
        }
        return final_list
    }
